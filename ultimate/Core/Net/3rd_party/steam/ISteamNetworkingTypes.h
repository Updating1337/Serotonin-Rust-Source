...
inline bool SteamNetworkingIPAddr::operator==(const SteamNetworkingIPAddr& x) const { return crt::memcmp(this, &x, sizeof(SteamNetworkingIPAddr)) == 0; }
...
inline bool SteamNetworkingIdentity::SetGenericString(const char* pszString) {
	size_t l = strlen(pszString); if (l >= sizeof(m_szGenericString)) return false;
...
inline bool SteamNetworkingIdentity::operator==(const SteamNetworkingIdentity& x) const { return m_eType == x.m_eType && m_cbSize == x.m_cbSize && crt::memcmp(m_genericBytes, x.m_genericBytes, m_cbSize) == 0; }
